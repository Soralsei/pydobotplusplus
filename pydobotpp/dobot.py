...
class Dobot:
    ...
    def _read_message(self) -> Optional[Message]:
        ...
        if begin_found:
            ...
            if len(payload_checksum) == payload_length + 1:
                ...
                self.logger.debug(
                    "pydobot: <<", ":".join("{:02x}".format(x) for x in b)
                )
                return msg
        return None
    ...
    def get_alarms(self) -> Set[Alarm]:
        ...
        for idx in range(16):
            ...
            for alarm_index in [
                i for i in range(alarm_byte.bit_length()) if alarm_byte & (1 << i)
            ]:
    ...
    def _set_queued_cmd_stop_exec(self, force=False):
        msg = Message()
        msg.id = 242 if force else 241
        msg.ctrl = 0x01
        return self._send_command(msg)
    ...
    def _set_jog_coordinate_params(
        self, vx, vy, vz, vr, ax=100, ay=100, az=100, ar=100
    ):
        msg = Message()
        msg.id = 71
        msg.ctrl = 0x03
    ...
    def speed(self, velocity=100.0, acceleration=100.0):
        self.wait_for_cmd(
            self._extract_cmd_index(self._set_ptp_common_params(velocity, acceleration))
        )
        self.wait_for_cmd(
    ...
    def _set_stepper_motor_distance(
        self, speed, distance, interface=0, motor_control=True
    ):
        msg = Message()
        msg.id = 0x88
        msg.ctrl = 0x03
    ...
    def engrave(
        ...
        self.wait_for_cmd(
            self._extract_cmd_index(
                self._set_cp_params(velocity, acceleration, actual_acceleration)
            )
        )
        ...
        for row_idx, row in enumerate(image):
            ...
            for col_idx, ld in enumerate(data):
                ...
                indexes.append(
                    self._extract_cmd_index(
                        self._set_cple_cmd(
                            x + row_idx * pixel_size, y + y_ofs, z, ld, True
                        )
                    )
                )
    ...
    def conveyor_belt_distance(
        ...
        MM_PER_REV = (
            34 * math.pi
        )  # Seems to actually be closer to 36mm when measured but 34 works better
        STEP_ANGLE_DEG = 1.8
        STEPS_PER_REV = (
            360.0 / STEP_ANGLE_DEG * 10.0 * 16.0 / 2.0
        )  # Spec sheet says that it can do 1.8deg increments, no idea what the 10 * 16 / 2 fuck factor is....
    ...
    def move_to(
        self, x=None, y=None, z=None, r=0.0, wait=True, mode=None, position=None
    ):
        if position is not None:
            x, y, z, r = position.x, position.y, position.z, position.r
        elif x is None and y is None and z is None:
            raise ValueError(
                "Either a Position object or x, y, z coordinates must be provided"
            )
